...
@test "Does the version show in the docker container?" {
...
@test "Can a simple file be loaded?" {
    output=$(./FLEcli load -i test/data/fle-1.txt)
    echo 'status:' $status
    echo 'output:' $output
}
...
@test "Is the generated SOTA csv equivalent to the canonical one" {
    ...
    output=$(./FLEcli csv -o -i test/FLE-sample/sota_wwff.txt test/output/temp/sota_wwff.csv)
    echo 'status:' $status
    echo 'output:' $output
